...
async def send_message(conversation_id: str,payload: MessageCreate) -> MessageResponse:
    ...
    try:
        ...
        tool_evidence_sources=[name for name,result in tool_results.items() if isinstance(result,dict) and result.get("evidence")]
        evidence_required=bool(plan.required or deep_plan.reasons)
        trace.evidence={"available":bool(evidence),"required":evidence_required,"source_count":len(search_results) + len(tool_evidence_sources),"tool_evidence_sources":tool_evidence_sources,"research_reasons":plan.reasons+[f"deep:{r}" for r in deep_plan.reasons]}
        ...
        activity_events.append("Seleccionando la mejor IA disponible…"); provider_context={**bounded_context,"conversation_id":conversation_id,"selected_tools":selected,"evidence":evidence,"evidence_source_count":len(search_results) + len(tool_evidence_sources),"tool_results":{k:{key:val for key,val in v.items() if key != "evidence"} if isinstance(v,dict) else v for k,v in tool_results.items()},"cost_mode":"free_only"}; answer=await providers.generate(messages=messages,context=provider_context)
