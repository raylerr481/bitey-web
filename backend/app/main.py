...
from .core.tool_orchestrator import ToolOrchestrator, ToolSpec
...
app = FastAPI(title="Bitey IA — Supracerebro Backend", version="0.7.0", description="General extensible intelligence with free-first tool and agent orchestration.", lifespan=lifespan)
...
async def web_research_tool(message: str, context: dict | None = None) -> dict:
    plan = deep_research.plan(message, context or {})
    plan = await deep_research.fetch(plan)
...
async def workspace_files_tool(message: str, context: dict | None = None) -> dict:
    return {"ok": True, "available": True, "note": "Workspace file metadata/context is handled by the project workspace layer."}

async def calculator_tool(message: str, context: dict | None = None) -> dict:
    return {"ok": True, "available": True, "note": "Use model reasoning for simple arithmetic; dedicated calculator integration can be added without changing orchestration."}
...
tools.register(ToolSpec("workspace_files", "Usa el contexto de archivos de proyectos.", ("files", "projects"), workspace_files_tool))
tools.register(ToolSpec("calculator", "Herramienta matemática sin proveedor de pago.", ("math",), calculator_tool))
tools.register(ToolSpec("code_reasoning", "Analiza código sin ejecutar código arbitrario.", ("code", "debug"), code_reasoning_tool))

@app.get("/health")
async def health() -> dict:
    return {"status":"ok","system":"bitey-ia-supracerebro","supabase_persistence":memory.persistent,"workspace_persistence":workspace.persistent,"learning_persistence":learning.persistent,"background_cognitive_engine":True,"deep_research":True,"tool_orchestration":True}

@app.get("/api/v1/capabilities")
async def capabilities() -> dict:
    return {"conversation":True,"dynamic_context":True,"memory":True,"persistent_memory":memory.persistent,"projects":True,"project_files_metadata":True,"web_research":True,"deep_research":True,"web_url_fetch":True,"feedback":True,"guarded_incremental_learning":learning.persistent,"background_cognitive_engine":True,"provider_orchestration":True,"tool_orchestration":True,"agent_orchestration":True,"tools":tools.available(),"cost_mode":"free_only","providers":providers.available()}
...
async def send_message(conversation_id: str,payload: MessageCreate) -> MessageResponse:
    ...
    if evidence: messages.insert(0,{"role":"system","content":"TOOL EVIDENCE — información pública recuperada por una herramienta de Bitey. Usa evidencia, no inventes. Señala contradicciones y separa hechos de inferencias.\n\n"+evidence})
    elif plan.required or deep_plan.reasons: messages.insert(0,{"role":"system","content":"La herramienta de investigación no recuperó evidencia utilizable. Decláralo y no inventes información."})
