...
async def _background_loop(stop_event: asyncio.Event) -> None:
    """Free-tier in-process background engine; jobs persist in Supabase."""
    while not stop_event.is_set():
        try:
            await process_once()
        except Exception:
            # Never allow background cognition to take down the API process.
            pass
        try:
...
app = FastAPI(
    ...
    version="0.5.0",
    description="General-purpose intelligence backend for Bitey IA with free-first orchestration, web research, memory, projects and guarded background learning.",
    lifespan=lifespan,
)
...
async def capabilities() -> dict:
    return {
        "conversation": True, "dynamic_context": True, "memory": True,
        "persistent_memory": memory.persistent, "projects": True,
        "project_files_metadata": True, "web_research": True,
        "web_url_fetch": True, "feedback": True,
        "guarded_incremental_learning": learning.persistent,
        "background_cognitive_engine": True,
        "provider_orchestration": True, "cost_mode": "free_only",
...
async def send_message(conversation_id: str, payload: MessageCreate) -> MessageResponse:
    ...
    if sources:
        ...
        if research_context:
            messages.insert(0, {"role": "system", "content": "Bitey IA puede realizar investigación web explícita. Usa únicamente las fuentes proporcionadas para hechos observados, distingue inferencias y no inventes contenido.\n\n" + research_context})

    answer = await providers.generate(messages=messages, context={**context.as_dict(), "research_plan": plan.__dict__, "cost_mode": "free_only"})
    await memory.append(conversation_id, {"role": "assistant", "content": answer})
    if learning.persistent:
        await learning.observe(title="conversation_observation", payload={"conversation_id": conversation_id, "message": payload.message, "answer": answer[:4000]}, source="conversation", confidence=0.4)

    return MessageResponse(conversation_id=conversation_id, answer=answer, research_required=plan.required, research_reasons=plan.reasons, providers=providers.available())
