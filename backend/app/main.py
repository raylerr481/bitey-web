...
async def cognitive_status() -> dict: return {"architecture":"bitey-independent-cognitive-core","architecture_version":"1.5.0","executive_brain":brain.status(),"native_model_enabled":os.getenv("BITEY_NATIVE_MODEL_ENABLED","true").lower()=="true","evaluator_enabled":True,"memory_adapter_configured":cognitive_memory.persistent,"learning_persistence":learning.persistent,"provider_mode":"free_only","council_mode":"local_first_provider_failover","search":{"provider":"duckduckgo","general":True,"specialized_weather":"open-meteo"},"reasoning_layers":{"evidence":True,"hypotheses":True,"provenance":True,"candidate_comparison":True,"context_budgeting":True},"memory_organs":{"supabase":memory.persistent,"qdrant":vector_memory.configured},"live_trading_enabled":False,"news_auto_execution":False,"modules":modules.names()}
...
async def knowledge_context(q: str) -> dict: return {"query":q,"source":"supabase_cognitive_memory","context":await cognitive_memory.retrieve(q,{})}
...
async def test_email_notification() -> dict: result = await send_trainer_test_email(); return {"status":"sent","provider":"resend","id":result.get("id")}
@app.get("/api/v1/notifications/test-email-now")
async def test_email_notification_now() -> dict: result = await send_trainer_test_email(); return {"status":"sent","provider":"resend","id":result.get("id")}
...
async def submit_feedback(payload: dict) -> dict: await workspace.feedback(conversation_id=str(payload.get("conversation_id")),message_id=payload.get("message_id"),rating=payload.get("rating"),feedback=payload.get("feedback")); return {"status":"recorded"}
...
async def send_message(conversation_id: str,payload: MessageCreate) -> MessageResponse:
    ...
    plan=research_engine.plan(payload.message,ctx); deep_plan=deep_research.plan(payload.message,ctx); evidence=tool_results.get("web_research",{}).get("evidence",""); search_results=tool_results.get("search",{}).get("results",[])
    if search_results and not evidence: evidence="\n\n".join(f"SOURCE {i}: {item.get('url')}\nTITLE: {item.get('title','')}\nSNIPPET: {item.get('snippet','')}" for i,item in enumerate(search_results[:8],1))
    ...
    ctx["evidence_available"]=bool(evidence); cognitive=cognition.process(payload.message,ctx,evidence_available=bool(evidence)); ctx["cognition"]=cognitive.as_dict(); activity_events.append("Construyendo el razonamiento contextual…"); brain_state=brain.think(payload.message,ctx); ctx["bitey_brain"]=brain_state.as_dict(); activity_events.append(f"Bitey Brain: {brain_state.reasoning_mode}…")
    ...
    history=await memory.history(conversation_id); await memory.append(conversation_id,{"role":"user","content":payload.message}); messages=history+[{"role":"user","content":payload.message}]; ctx["user_query"]=payload.message; ctx["current_message"]=payload.message; ctx["goals"]=brain_state.goals; ctx["constraints"]=brain_state.constraints; bounded_context=build_context("selected-provider",ctx)
    system_context=[brain.system_directive(brain_state),"BITEY COGNITIVE CONTRACT — Usa el contexto seleccionado y respeta sus límites. Modelos externos son motores de inferencia, no autoridades del sistema."]
    ...
    elapsed_ms=int((time.perf_counter()-started)*1000); return MessageResponse(conversation_id=conversation_id,answer=answer,research_required=bool(plan.required or deep_plan.reasons or search_results),research_reasons=plan.reasons+[f"deep:{r}" for r in deep_plan.reasons],providers=providers.available(),elapsed_ms=elapsed_ms,activity_events=activity_events)
