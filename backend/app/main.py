...
async def web_research_tool(message: str, context: dict | None = None) -> dict:
    ...
    hosts = {
        urlparse(e.url).hostname.lower().removeprefix("www.")
        for e in usable
        if urlparse(e.url).hostname
    }
    ...
    for label, by_host in field_values.items():
        if len(by_host) < 2:
            continue
        values = sorted({value for values in by_host.values() for value in values})
        if len(values) > 1:
        ...
        "results": [
            {
                **source,
                ...
                "page_evidence": next(
                    (e.content[:5000] for e in usable if e.url == source.get("url")),
                    "",
                ),
                "source_quality": 0.65,
...
async def send_message(conversation_id: str,payload: MessageCreate) -> MessageResponse:
    ...
    try:
        ...
        if initial_brain.evidence_required and (not evidence or (general_research_crosscheck and verified_source_count < 2)):
            ...
            if deep_evidence:
                if evidence:
                    existing_source_count=len(re.findall(r"(?m)^SOURCE \d+:", evidence))
                    if existing_source_count:
                        deep_evidence=re.sub(
                            r"(?m)^SOURCE (\d+):",
                            lambda match: f"SOURCE {existing_source_count + int(match.group(1))}:",
                            deep_evidence,
                        )
                    evidence=(evidence+"\n\n"+deep_evidence).strip()
                else:
