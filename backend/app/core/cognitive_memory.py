...
class CognitiveMemoryAdapter:
    ...
    Memory is fail-closed for scoped executions: a request without a server-produced
    memory scope receives no persisted cognitive rows. This prevents unscoped legacy
    rows from becoming an accidental cross-capability context source.
    """
    ...
    def _headers(self) -> dict[str, str]:
        return {"apikey": self.key, "Authorization": f"Bearer {self.key}", "Accept": "application/json"}
    ...
    def _filter_scope(cls, rows: list[dict[str, Any]], memory_scope: str | None) -> list[dict[str, Any]]:
        """Only rows carrying the exact server-produced scope may enter cognition."""
        if not memory_scope:
            return []
    ...
    async def retrieve(self, message: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not memory_scope:
            return {"enabled": True, "available": False, "records": {}, "memory_scope": None, "summary": "Cognitive memory withheld because execution scope is unavailable."}
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                for table in self.TABLES:
                    try:
                        rows = self._filter_scope(await self._read_table(client, table), memory_scope)
            ...
            return {"enabled": True, "available": False, "records": {}, "errors": {"connection": type(exc).__name__}, "memory_scope": memory_scope, "summary": "Cognitive memory unavailable; continuing without learned context."}

        counts = {table: len(rows) for table, rows in records.items()}
        available = bool(records)
        return {"enabled": True, "available": available, "records": records, "counts": counts, "errors": errors, "memory_scope": memory_scope, "summary": "Learned cognitive context retrieved." if available else "No learned cognitive context available yet."}
    ...
    def compact_for_prompt(memory: dict[str, Any], max_chars: int = 5000) -> str:
        ...
        for table, rows in (memory.get("records") or {}).items():
            for row in rows[:5]:
                ...
                chunks.append(f"{table}: {compact}"[:900])
