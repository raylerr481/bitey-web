...
class CognitiveMemoryAdapter:
    ...
    carry the same ``memory_scope``. Missing scope is fail-closed whenever a context
    was supplied, preventing unscoped learned data from entering a scoped request.
    """
    ...
    def _filter_scope(cls, rows: list[dict[str, Any]], memory_scope: str | None) -> list[dict[str, Any]]:
        """Apply fail-closed isolation whenever a scope is required."""
        if not memory_scope:
            return []
    ...
    async def retrieve(self, message: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if context is not None and not memory_scope:
            return {
                "enabled": True,
                "available": False,
                "records": {},
                "memory_scope": None,
                "summary": "Cognitive memory scope unavailable; continuing without learned context.",
            }
