...
class ExecutiveEvaluator:
    ...
    def evaluate(self, *, state: Any, answer: str, evidence: str = "", selected_tools: list[str] | None = None) -> ExecutiveEvaluation:
        ...
        refusal = risk == "critical" and not execution_allowed and any(x in text.lower() for x in ("no se ejecutará", "no se ejecutara", "bloqueada", "blocked", "will not execute"))
        verification_ok = not verification_required or bool(evidence) or refusal
        ...
        return ExecutiveEvaluation("accept" if passed else "revise", passed, evidence_ok, tool_ok, risk_ok, verification_ok, provider_independent, reasons or ["executive_contract_satisfied"])
