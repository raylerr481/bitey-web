...
class AutonomousLanguageOrchestrator:
    """Language-first planner; runtime registry remains capability authority."""
    ...
    def plan(self, message: str, context: dict[str, Any] | None = None) -> LanguagePlan:
        ...
        return LanguagePlan(intent,domain,tuple(capabilities),external,freshness,verification,objective,confidence,tuple(dict.fromkeys(reasons)),strategy)
    def status(self)->dict[str,Any]:
        return {"name":"Autonomous Language Orchestrator","version":"2.0.0","mode":"language_first_adaptive","semantic_planning":True,"model_is_advisor":True,"runtime_registry_is_authority":True,"adaptive_search":True,"no_fixed_result_cap":True,"multilingual":True}
