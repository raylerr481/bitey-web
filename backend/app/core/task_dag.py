...
class TaskDAG:
    ...
    def _assert_acyclic(self) -> None:
        graph = {node.id: node.depends_on for node in self.nodes}; visiting: set[str] = set(); visited: set[str] = set()
        def visit(node_id: str) -> None:
            if node_id in visiting: raise ValueError("task_dag_cycle_detected")
    ...
    def depth(self) -> int:
        graph = {node.id: node.depends_on for node in self.nodes}; memo: dict[str, int] = {}
        def level(node_id: str) -> int:
            if node_id in memo: return memo[node_id]
            memo[node_id] = 1 + max((level(dep) for dep in graph[node_id]), default=0)
            return memo[node_id]
    ...
    def mark_completed(self, node_id: str, result: Any = None) -> None:
        for node in self.nodes:
            if node.id == node_id:
                node.status = "completed"; node.result = result; self.validate(); return
...
def dag_from_plan(plan: list[dict[str, Any]]) -> TaskDAG:
    """Build a bounded plan while preventing an unbounded dependency chain."""
    nodes: list[TaskNode] = []
    ...
    for index, step in enumerate(plan[:MAX_NODES], start=1):
        ...
        elif previous is not None and index <= MAX_DEPTH:
            deps = [previous]
        else:
            # Once the safety depth is reached, start an independent bounded
            # branch rather than silently constructing a deeper chain.
            deps = []
