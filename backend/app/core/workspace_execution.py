...
class WorkspaceExecutionService:
    ...
    @classmethod
    def _resolve_capability(cls,prompt,requested,state):
    ...
    def _build_contract(self,*,prompt,requested_capability,resolved_capability,state,decision,context):
        artifact_type=self.ARTIFACT_CAPABILITIES.get(resolved_capability); contract=TaskContract(prompt=prompt,intent=str(getattr(state,"task_class","conversation")),capability=resolved_capability,constraints=list(getattr(state,"constraints",[]) or []),risk_level=str(getattr(state,"risk_level","low")),budget={"paid_inference":False,"max_retries":int(context.get("max_retries",2))},plan=[{"step":1,"action":"cognitive_decision","owner":"bitey_ia"}],evidence_policy={"required":bool(getattr(state,"evidence_required",False)),"freshness":bool(getattr(state,"freshness_required",False))},evaluation_policy={"required":True,"decision_owner":"bitey_ia"},artifact_contract={"type":artifact_type,"required":bool(artifact_type)} if artifact_type else None,authorization={"side_effects":bool(getattr(state,"execution_allowed",False)),"required_for_external_actions":True})
        contract.plan += [{"step":2,"action":"resolve_capability","requested":requested_capability,"resolved":resolved_capability},{"step":3,"action":"research_if_required","bounded":True,"max_steps":4},{"step":4,"action":"worker_inference","paid_forbidden":True},{"step":5,"action":"evaluation","required":True}]; contract.validate(); return contract
    async def execute(self,*,prompt,capability,context=None):
        ctx=dict(context or {}); requested_capability=(capability or "chat").strip().lower(); state=self.brain.think(prompt,ctx); decision=state.as_dict(); resolved_capability,resolution_reason=self._resolve_capability(prompt,requested_capability,state); decision.update({"workspace_capability":resolved_capability,"requested_capability":requested_capability,"capability_resolution":resolution_reason}); contract=self._build_contract(prompt=prompt,requested_capability=requested_capability,resolved_capability=resolved_capability,state=state,decision=decision,context=ctx); evidence=""; research_result=None
        if resolved_capability in {"deep_research","browser_research"} or state.evidence_required:
            contract.transition("researching"); contract.plan.append({"step":6,"action":"bounded_research","max_steps":4,"max_sources_per_step":5}); research_result_obj=await self.research.run(prompt,ctx); research_result=research_result_obj.as_dict(); evidence=str(research_result.get("evidence_context") or getattr(research_result_obj,"evidence_context","")); decision={**decision,**getattr(research_result_obj,"decision",{}),"workspace_capability":resolved_capability,"requested_capability":requested_capability,"capability_resolution":resolution_reason}
        ...
        contract.transition("evaluating"); evaluation=self.evaluator.evaluate(user_message=prompt,answer=answer,context={**ctx,"evidence_required":state.evidence_required,"domain":state.task_class},evidence=evidence); artifact_type=self.ARTIFACT_CAPABILITIES.get(resolved_capability); artifact=None
        if artifact_type and evaluation.decision=="accept":
            contract.artifact_contract={"type":artifact_type,"required":True}; artifact=build_artifact(name=self._artifact_name(prompt,artifact_type),artifact_type=artifact_type,content=self._artifact_content(answer,artifact_type),metadata={"capability":resolved_capability,"requested_capability":requested_capability,"capability_resolution":resolution_reason,"evaluation":evaluation.as_dict(),"cognitive_decision":decision,"task_contract":contract.to_dict(),"evidence_present":bool(evidence),"owner":"bitey_ia"})
        final_status="completed" if evaluation.decision=="accept" else "needs_review"; contract.transition(final_status,reason=None if final_status=="completed" else "evaluation_requires_review")
    ...
    @staticmethod
    def _artifact_name(prompt,artifact_type):
    ...
    @staticmethod
    def _artifact_content(answer,artifact_type):
