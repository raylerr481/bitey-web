...
class FakeResearchEngine:
    ...
    def plan(self, query, context):
        return DeepResearchPlan(query=query, reasons=list(self.reasons), mode="deep")
    ...
    async def fetch(self, plan):
        ...
        plan.evidence.append(
            Evidence(
                ...
                title="Test evidence",
                content=f"Evidence for {plan.query}",
                ok=True,
            )
        )
        return plan
...
class MultiStepResearchRuntimeTests(unittest.IsolatedAsyncioTestCase):
    async def test_no_research_intent_is_deterministic_and_does_not_call_engine(self):
        ...
        runtime = MultiStepResearchRuntime(engine, max_subquestions=4, max_passes=2)
    ...
    async def test_subquestion_limit_is_global(self):
        ...
        runtime = MultiStepResearchRuntime(engine, max_subquestions=3, max_passes=5)

        result = await runtime.run("Investigate this", {})
    ...
    async def test_pass_limit_is_enforced(self):
        engine = FakeResearchEngine()
        runtime = MultiStepResearchRuntime(engine, max_subquestions=10, max_passes=1)
