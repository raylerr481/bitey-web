...
class TestWorkspaceTaskAutoRecovery(unittest.TestCase):
    def test_stale_running_task_is_requeued_once_and_recovered(self):
        ...
        class FakeClient:
            ...
            async def patch(self, *args, **kwargs):
                ...
                return FakeResponse([dict(task)])

        async def fake_execute(workspace_id, task_id, recovered):
            recovered["status"] = "completed"
            return recovered

        with patch("app.background_worker.SUPABASE_URL", "https://example.supabase.co"), patch(
            "app.background_worker.SUPABASE_KEY", "test-key"
        ), patch("app.background_worker.httpx.AsyncClient", return_value=FakeClient()), patch(
        ...
        self.assertEqual(result, {"scanned": 1, "recovered": 1, "skipped": 0})
    ...
    def test_fresh_running_task_is_not_recovered(self):
        ...
        task = {"id": "task-2", "workspace_id": "ws-1", "status": "running", "updated_at": fresh, "metadata": {"task_dag": {"version": 1, "nodes": []}}}
        ...
        self.assertEqual(result, {"scanned": 1, "recovered": 0, "skipped": 1})
