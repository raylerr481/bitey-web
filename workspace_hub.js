(() => {
  ...
  async function ensureWorkspace(existing){
    if(existing?.length) return existing[0];
    const created=await createWorkspace('Espacio general');
    return created || null;
  }
  function taskForm(cap, workspace) {
    ...
    document.getElementById('workspace-run')?.addEventListener('click',async()=>{
      ...
      if(!task){open(cap.label,'No se pudo crear la tarea.','<p class="muted">Comprueba que el backend de Bitey IA Web esté disponible y que el Workspace esté configurado.</p>');return;}
      const result=await runTask(workspace.id,task.id);
      if(!result){open(cap.label,'La tarea quedó registrada pero no pudo ejecutarse.','<p class="muted">La tarea permanece en el Workspace para poder inspeccionarla o reintentarse.</p>');return;}
  ...
  function renderTaskResult(cap,task){
    ...
    const evaluationHtml=evaluation.decision?`<div class="workspace-research-meta"><b>Evaluación</b><span>${esc(evaluation.decision)}${typeof evaluation.confidence==='number'?` · confianza ${Math.round(evaluation.confidence*100)}%`:''}</span></div>`:'';
    open(cap.label,meta.join(' · ')||'Resultado de Bitey IA',`${artifactHtml}${researchHtml}${evaluationHtml}<div class="workspace-answer">${esc(answer||'Bitey no devolvió contenido utilizable.')}</div><div class="workspace-form-actions"><button class="primary-action" id="workspace-done">Cerrar</button></div>`);
  ...
  async function hub(){
    const [c,ws0]=await Promise.all([catalog(),workspaces()]);
    const current=await ensureWorkspace(ws0);
    const ws=current ? [current,...ws0.filter(x=>x.id!==current.id)] : ws0;
    ...
    open('Bitey IA Workspace','Un workspace integral inspirado en la experiencia de las plataformas modernas de IA, implementado dentro de Bitey IA Web.',`<div class="workspace-status"><span class="workspace-status-dot"></span><span>${status.free_only!==false?'Modo gratuito protegido':'Modo de ejecución configurado'}</span><span>·</span><span>${status.paid_fallback===false?'sin fallback de pago':'política definida'}</span></div><div class="workspace-section-title">Capacidades</div><div class="workspace-grid">${renderCards(caps)}</div><div class="workspace-section-title">Espacios de trabajo</div><div class="workspace-list"><div class="workspace-current"><span class="workspace-icon">◇</span><div><b>${esc(current?.name||'Espacio general')}</b><small>${ws.length?`${ws.length} espacios disponibles`:'Workspace local'}</small></div></div><div class="workspace-form-actions"><button class="secondary-action" id="workspace-refresh">Actualizar</button><button class="primary-action" id="new-workspace">＋ Nuevo espacio</button></div></div>`);
    body()?.querySelectorAll('[data-cap]').forEach(b=>b.addEventListener('click',()=>{const cap=caps.find(x=>x.id===b.dataset.cap)||{id:b.dataset.cap,label:b.dataset.cap}; if(!current){open('Workspace','No se pudo crear el espacio general.','<p class="muted">El backend de Bitey IA Web no pudo crear un Workspace.</p>');return;} taskForm(cap,current);}));
    document.getElementById('workspace-refresh')?.addEventListener('click',hub);
    document.getElementById('new-workspace')?.addEventListener('click',async()=>{const n=window.prompt('Nombre del espacio de Bitey IA');if(!n?.trim())return;const created=await createWorkspace(n.trim());if(created)hub();});
  }
