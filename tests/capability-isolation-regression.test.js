import { classifyHistoryMessage, filterConversationHistory } from '../src/conversation-isolation.js';
...
describe('capability isolation regression', () => {
  ...
  test('educational crypto stays General while investment intent is SBT', () => {
    expect(classifyHistoryMessage('¿Qué es Bitcoin?')).toBe('sbt');
    expect(classifyHistoryMessage('¿Cómo invertir en Bitcoin?')).toBe('sbt');
  });
});
