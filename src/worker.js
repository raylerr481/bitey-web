...
async function runEdgeAiDiagnostic(env, requestId) {
  ...
  } catch (error) { return jsonError('Workers AI model execution failed', 502, requestId); }
}

async function tryRealAiFallback(upstream, request, env, requestId, origin) {
  if (!request) return null;
  let degraded = !upstream.ok; let upstreamBody = null;
  try {
    const raw = await upstream.clone().text(); try { upstreamBody = JSON.parse(raw); } catch (_) {}
    const answer = String(upstreamBody?.answer || '').trim(); const providers = Array.isArray(upstreamBody?.providers) ? upstreamBody.providers : [];
...
async function enrichSuccessfulResponse(upstream, request, requestId) {
  ...
  let payload; try { payload = await request.clone().json(); } catch (_) { return null; }
  const message = String(payload?.message || '').trim(); if (!message || (!WEATHER_RE.test(message) && !RESEARCH_RE.test(message))) return null;
  const evidence = await recoverToolEvidence(message, requestId); const sources = Array.isArray(evidence?.sources) ? evidence.sources : []; if (!sources.length) return null;
  let body; try { body = JSON.parse(await upstream.clone().text()); } catch (_) { return null; } if (Array.isArray(body?.sources) && body.sources.length) return null;
  body.sources = sources; if (RESEARCH_RE.test(message)) { body.research_required = true; if (!Array.isArray(body.research_reasons) || !body.research_reasons.length) body.research_reasons = ['evidence_recovery']; }
  const headers = new Headers(upstream.headers); headers.set('Content-Type', 'application/json; charset=utf-8'); headers.set('Cache-Control', 'no-store'); headers.set('X-Bitey-Edge', 'cloudflare-ai-evidence-enrichment'); headers.set('X-Bitey-Request-Id', requestId);
...
async function runRealAiFallback(request, env, requestId, cause, origin, upstreamBody = null) {
  if (!env.AI) return null; let payload;
  try { payload = await request.clone().json(); } catch (error) { return null; }
  const message = String(payload?.message || '').trim(); const conversationId = String(request.url).match(/conversations\/([^/]+)\/messages/)?.[1] || ''; if (!message) return null;
  const capability = resolveFallbackCapability(payload, message);
  if (capability !== 'general') return specializedFallbackBlocked(capability, requestId);
  let history = []; if (conversationId) history = await loadConversationHistory(origin, conversationId, requestId);
  ...
  const backendSources = backendEvidenceCapability && backendEvidenceCapability !== 'general' ? [] : (Array.isArray(upstreamBody?.sources) ? upstreamBody.sources : []); const sources = backendSources.length ? backendSources : (Array.isArray(evidence?.sources) ? evidence.sources : []);
  const evidenceInstruction = combinedEvidence ? `EVIDENCIA RECUPERADA POR BITEY:\n${combinedEvidence}\n\nUsa esta evidencia para responder. No inventes datos y no menciones herramientas internas.` : (RESEARCH_RE.test(message) ? 'La consulta puede requerir información externa. Si no hay evidencia recuperada, no inventes datos; explica brevemente la limitación.' : '');
  const sourceInstruction = sources.length ? `FUENTES CONSULTADAS:\n${sources.map((s, i) => `[${i + 1}] ${s.title || s.url || 'Fuente'} — ${s.url || ''}`).join('\n')}\n\nCuando afirmes datos procedentes de estas fuentes, cita [1], [2], etc. No inventes referencias.` : '';
  ...
  const attempts = [{ messages, max_tokens: 512 }, { messages: [{ role: 'system', content: system }, ...(evidenceInstruction ? [{ role: 'system', content: evidenceInstruction }] : []), ...(sourceInstruction ? [{ role: 'system', content: sourceInstruction }] : []), { role: 'user', content: message }], max_tokens: 512 }];
  for (let index = 0; index < attempts.length; index++) {
    try { const response = await env.AI.run(AI_MODEL, { messages: attempts[index].messages, max_tokens: attempts[index].max_tokens, temperature: 0.2, chat_template_kwargs: { enable_thinking: false } }); const answer = extractAiText(response); if (!answer) throw new Error('empty_response'); return jsonResponse({ conversation_id: conversationId, answer, research_required: Boolean(combinedEvidence), research_reasons: combinedEvidence ? ['evidence_recovery'] : [], providers: [AI_MODEL], selected_provider: 'cloudflare-workers-ai', elapsed_ms: null, activity_events: [combinedEvidence ? 'Respuesta final generada por un modelo de lenguaje real usando evidencia recuperada por Bitey.' : 'Generación de recuperación realizada por un modelo de lenguaje real de Cloudflare Workers AI.'], sources, request_id: requestId }, 200, 'cloudflare-ai-fallback', requestId); } catch (error) { console.error('Bitey Workers AI fallback attempt failed', { requestId, attempt: index + 1, cause: String(cause), error: String(error) }); }
  }
  return null;
}
...
function specializedFallbackBlocked(capability, requestId) {
  const answer = capability === 'sbt' ? 'La capacidad de SBT no está disponible en este momento. No voy a simular una respuesta de trading o inversión.' : capability === 'enterprise' ? 'La capacidad de Bitey Enterprise no está disponible en este momento. No voy a simular una respuesta especializada de marketing o crecimiento empresarial.' : 'La capacidad de JobIA no está disponible en este momento. No voy a simular una respuesta especializada de empleo.';
...
async function recoverToolEvidence(message, requestId) {
  try { if (WEATHER_RE.test(message)) return await recoverWeather(message, requestId); if (RESEARCH_RE.test(message)) return await recoverSearch(message, requestId); } catch (error) { console.warn('Bitey edge evidence recovery failed', { requestId, error: String(error) }); } return null;
}

function weatherLocation(message) { const known = message.match(/\b(esteio|porto alegre)\b/i); if (known) return known[1]; const match = message.match(/(?:en|in|em|de|da|do)\s+(.+?)(?:,\s*(?:brasil|brazil))?(?:[?!.]|$)/i); return (match?.[1] || '').replace(/\b(?:rio grande do sul|rs|estado de)\b/ig, '').replace(/\s+/g, ' ').trim() || null; }

async function recoverWeather(message, requestId) {
  const locationQuery = weatherLocation(message); if (!locationQuery) return null; const geoUrl = new URL('https://geocoding-api.open-meteo.com/v1/search'); geoUrl.searchParams.set('name', locationQuery); geoUrl.searchParams.set('count', '5'); geoUrl.searchParams.set('language', 'pt'); geoUrl.searchParams.set('format', 'json'); const geoResponse = await fetch(geoUrl, { headers: { 'User-Agent': 'BiteyWeb/1.0' } }); if (!geoResponse.ok) return null; const locations = (await geoResponse.json())?.results || []; if (!locations.length) return null; const location = locations.find(x => String(x?.name || '').toLowerCase() === locationQuery.toLowerCase()) || locations[0]; const weatherUrl = new URL('https://api.open-meteo.com/v1/forecast'); weatherUrl.searchParams.set('latitude', String(location.latitude)); weatherUrl.searchParams.set('longitude', String(location.longitude)); weatherUrl.searchParams.set('current', 'temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code'); weatherUrl.searchParams.set('timezone', 'auto'); weatherUrl.searchParams.set('forecast_days', '1'); const weatherResponse = await fetch(weatherUrl, { headers: { 'User-Agent': 'BiteyWeb/1.0' } }); if (!weatherResponse.ok) return null; const current = (await weatherResponse.json())?.current || {}; return { text: `WEATHER SOURCE: Open-Meteo\nLOCATION: ${location.name}, ${location.admin1 || ''}, ${location.country || ''}\nOBSERVATION TIME: ${current.time || 'unknown'}\nTEMPERATURE: ${current.temperature_2m ?? 'unknown'} °C\nAPPARENT TEMPERATURE: ${current.apparent_temperature ?? 'unknown'} °C\nHUMIDITY: ${current.relative_humidity_2m ?? 'unknown'} %\nWIND: ${current.wind_speed_10m ?? 'unknown'} km/h\nWEATHER CODE: ${current.weather_code ?? 'unknown'}`, sources: [{ title: 'Open-Meteo', url: weatherUrl.toString(), snippet: `Datos meteorológicos actuales de ${location.name}. Observación: ${current.time || 'unknown'}.` }] };
}

async function recoverSearch(message, requestId) {
  const sources = [{ base: 'https://html.duckduckgo.com/html/', selector: /<div class=\"result__body\".*?<\/div>\s*<\/div>/gs, link: /class=\"result__a\"[^>]*href=\"([^\"]+)\"[^>]*>(.*?)<\/a>/s, snippet: /class=\"result__snippet\"[^>]*>(.*?)<\/(?:a|div)>/s }, { base: 'https://lite.duckduckgo.com/lite/', selector: /<tr>\s*<td[^>]*class=\"result-link\"[\s\S]*?<\/tr>/gi, link: /<a[^>]+rel=\"nofollow\"[^>]+href=\"([^\"]+)\"[^>]*>(.*?)<\/a>/i, snippet: /class=\"result-snippet\"[^>]*>(.*?)<\//i }];
  for (const source of sources) { try { const url = new URL(source.base); url.searchParams.set('q', message); const response = await fetch(url, { headers: { 'User-Agent': 'BiteySearch/1.0', 'Accept': 'text/html' } }); if (!response.ok) continue; const body = await response.text(); const blocks = [...body.matchAll(source.selector)].slice(0, 8); const items = []; const sourceObjects = []; for (const blockMatch of blocks) { const block = blockMatch[0]; const link = block.match(source.link); if (!link) continue; const raw = decodeHtml(link[1]); const redirect = raw.match(/[?&]uddg=([^&]+)/); const target = redirect ? decodeURIComponent(redirect[1]) : raw; const title = stripHtml(decodeHtml(link[2])); const snippetMatch = block.match(source.snippet); const snippet = stripHtml(decodeHtml(snippetMatch?.[1] || '')); if (/^https?:\/\//i.test(target) && title) { sourceObjects.push({ title, url: target, snippet }); items.push(`SOURCE ${sourceObjects.length}: ${target}\nTITLE: ${title}\nSNIPPET: ${snippet}`); } } if (items.length) return { text: items.join('\n\n'), sources: sourceObjects }; } catch (error) { console.warn('Bitey edge search source failed', { requestId, source: source.base, error: String(error) }); } } return null;
}
...
async function loadConversationHistory(origin, conversationId, requestId) { if (!origin || !conversationId) return []; try { const url = new URL(`/api/v1/conversations/${encodeURIComponent(conversationId)}/messages`, origin); const response = await fetch(url, { method: 'GET', headers: { 'Accept': 'application/json', 'x-bitey-channel': 'web', 'x-bitey-origin': 'cloudflare', 'x-request-id': requestId } }); if (!response.ok) return []; const body = await response.json(); return Array.isArray(body?.messages) ? body.messages.slice(-8) : []; } catch (error) { return []; } }
...
function jsonResponse(body, status = 200, source = 'cloudflare', requestId = '') { return new Response(JSON.stringify(body), { status, headers: { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store', 'X-Bitey-Edge': source, 'X-Bitey-Request-Id': requestId } }); }
function jsonError(message, status = 500, requestId = '') { return jsonResponse({ error: message, request_id: requestId }, status, 'cloudflare-error', requestId); }
