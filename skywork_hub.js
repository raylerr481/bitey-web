(() => {
  ...
  async function hub(){
    ...
    open('Workspace de Bitey IA','Un espacio de trabajo integral para investigar, crear y ejecutar tareas con el cerebro cognitivo de Bitey.',
      `<div class="skywork-grid">${caps.map(x=>`<button class="skywork-card" data-cap="${esc(x.id)}"><span class="skywork-icon">${x.id==='deep_research'?'✦':x.id==='documents'?'▤':x.id==='slides'?'▥':x.id==='spreadsheets'?'▦':x.id==='code'?'⌘':x.id==='files'?'▣':x.id==='agents'?'◈':'◌'}</span><b>${esc(x.label)}</b><small>${esc(x.kind)}</small></button>`).join('')}</div><div class="skywork-workspaces"><div class="feature-row"><span>◇</span><span><b>${esc(current?.name || 'Espacio general')}</b><small>${ws.length ? `${ws.length} espacios disponibles` : 'Crea tu primer espacio de trabajo'}</small></span></div><button class="primary-action" id="sky-new-workspace">＋ Nuevo espacio</button></div>`);
    ...
    document.getElementById('sky-new-workspace')?.addEventListener('click',async()=>{ const n=window.prompt('Nombre del espacio'); if(!n?.trim())return; await createWorkspace(n.trim()); hub(); });
  }
